...
class SodiumUtilsIo extends SodiumUtils {
  ...
  KeyPair publicKey(Uint8List sk) {
    ...
    var key = s.Sodium.cryptoSignSeedKeypair(seed);
    return KeyPair(pk: key.pk, sk: key.sk);
  }

  @override
  KeyPair cryptoSignSeedKeypair(Uint8List seed) {
    var keypair = s.Sodium.cryptoSignSeedKeypair(seed);
    return KeyPair(sk: keypair.sk, pk: keypair.pk);
  }
...
SodiumUtils getSodium() => SodiumUtilsIo();
